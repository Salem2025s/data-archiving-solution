"""MongoDB extraction script for the `classificationDefinition` collection.

This module extracts documents from MongoDB
(`DDTMdbDemo.classificationDefinition`) and returns normalized rows as
``list[dict]`` for V1 full-reload pipelines.
"""
...
def extract_classification_definition() -> list[dict[str, Any]]:
    """Extract and normalize documents from ``classificationDefinition``.
    ...
    try:
        ...
        documents = client.find_many(collection_name="classificationDefinition")
        ...
        logger.info(
            "Extraction completed for classificationDefinition: {} row(s)",
            len(rows),
        )
        return rows
        ...
        logger.exception(
            "Extraction failed for Mongo collection classificationDefinition"
        )
        raise
    finally:
        client.close()
...
def main() -> None:
    ...
    configure_logging()
    rows = extract_classification_definition()
