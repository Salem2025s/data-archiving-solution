...
3. **Labels = consensus LLM local** (`qwen2.5-32b-instruct`, 2 variantes de prompt, consensus ≥ 0,7). **Un expert métier a validé un échantillon des labels générés et confirmé leur bonne qualité.** Les métriques du holdout portent sur ces labels ; les prédictions incertaines sont routées en revue (`review_required`) avant toute décision.
